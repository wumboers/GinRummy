...
def render_state(model: dict[str, Any], context: dict[str, Any]) -> None:
    ...
    can_draw_stock = state["turn"] == you and state["stage"] in ("draw", "offer_first_upcard")
    can_draw_discard = state["turn"] == you and (
        state["stage"] == "draw" or (state["stage"] == "offer_first_upcard" and state["offered_to"] == you)
    )

    stock_box = model["stock_box"]
    discard_box = model["discard_box"]
    stock_label_color = "#ffd54f" if can_draw_stock else "white"
    discard_label_color = "#ffd54f" if can_draw_discard else "white"
    ...
    draw_card(canvas, stock_box[0], stock_box[1], "XX", selected=False, hidden=True, clickable=can_draw_stock)
    ...
    if state["discard_top"]:
        draw_card(canvas, discard_box[0], discard_box[1], state["discard_top"], selected=False, hidden=False, clickable=can_draw_discard)
    else:
    ...
    canvas.create_text(300, 500, text=f"Deadwood: {state['your_deadwood']}", fill=deadwood_color, font=("Segoe UI", 13, "bold"), anchor="w")
    canvas.create_text(430, 500, text="Green = melded   Gold = newest draw", fill="#d1fae5", font=("Segoe UI", 10, "bold"), anchor="w")
    ...
    if state["pending_knock"]:
        canvas.create_text(720, 500, text="Knock Armed", fill="#ffd54f", font=("Segoe UI", 14, "bold"))
...
def render_round_summary(canvas: tk.Canvas, state: dict[str, Any]) -> None:
    ...
    canvas.create_rectangle(70, 160, 810, 500, fill="#102a43", outline="#ffd166", width=3)
    ...
    canvas.create_text(110, 330, text=f"{your_name} revealed hand", fill="white", font=("Segoe UI", 12, "bold"), anchor="w")
    for index, card in enumerate(y_cards):
        draw_card(canvas, 110 + index * (CARD_WIDTH - 10), 346, card, selected=False)
