...
def draw_card(canvas: tk.Canvas, x: int, y: int, card: str, selected: bool, hidden: bool = False) -> None:
    ...
    """
    outline = "#ffd54f" if selected else "#1b5e20"
    width = 3 if selected else 2
    fill = "#f7f7f7" if not hidden else "#1e88e5"
...
def render_state(model: dict[str, Any], context: dict[str, Any]) -> None:
    ...
    canvas.create_text(300, 500, text=f"Deadwood: {state['your_deadwood']}", fill="#fff3cd", font=("Segoe UI", 13, "bold"), anchor="w")
    your_boxes = hand_hitboxes(state["your_hand"], 120, 525)
    model["your_boxes"] = your_boxes
    for card, box in your_boxes:
        x1, y1, _, _ = box
        draw_card(canvas, x1, y1, card, selected=context.get("selected_card") == card, hidden=False)
